...
class TestKafkaWhitebox(unittest.TestCase):
    ...
    def test_normal_use(self, requests_mock):
        ...
        # Deployment Manager Produces - URL not found (404)
        response = mocked_requests_get(404, '')
        requests_mock.return_value = response
        plugin = DMBlackBox()
        values = plugin.runner("--dmendpoint http://localhost", True)
        health = values[-1]
        ...
        # Deployment Manager Produces - Internal Server Error (500)
        response = mocked_requests_get(500, '500 - Internal Server Error')
        requests_mock.return_value = response
        plugin = DMBlackBox()
        values = plugin.runner("--dmendpoint http://localhost", True)
        health = values[-1]
        ...
        response = mocked_requests_get(503, 'Unable to connect to the Package Repository Manager')
        requests_mock.return_value = response
        plugin = DMBlackBox()
        values = plugin.runner("--dmendpoint http://localhost", True)
        health = values[-1]
        ...
        # Package repository Manager Produces - URL not found (404)
        response = mocked_requests_get(404, 'Package Repository Manager - URL Not Found: /packages.')
        requests_mock.return_value = response
        plugin = DMBlackBox()
        values = plugin.runner("--dmendpoint http://localhost", True)
        health = values[-1]
        ...
        # Package repository Manager produces - internal server error ( 500 )
        response = mocked_requests_get(500, 'Package Repository Manager - Internal Server Error')
        requests_mock.return_value = response
        plugin = DMBlackBox()
        values = plugin.runner("--dmendpoint http://localhost", True)
        health = values[-1]
