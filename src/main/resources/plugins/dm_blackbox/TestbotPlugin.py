...
class DMBlackBox(PndaPlugin):
    ...
    def validate_api_response(response, path):
        ...
        if response.status_code in expected_codes:
            return 'SUCCESS', None
        elif response.status_code in [500, 503, 404, 400, 409]:
            if response.status_code == 500:
                try:
                    json_obj = json.loads(response.text)
                    temp_msg = json_obj.get('information', 'Not Available')
                except ValueError:
    ...
    def runner(self, args, display=True):
        ...
        try:
            ...
            if status == 'SUCCESS':
                ...
                if 'Package Repository Manager' not in msg:
            ...
            cause.append('Unable to connect to the Deployment Manager')

        except Exception:
            cause.append('Exception - ' + str(sys.exc_info()[0]))
